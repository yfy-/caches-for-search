...
constexpr std::uint32_t CACHE_SIZE = 67504;
...
//  constexpr std::uint32_t unique_size = 62;
  ...
  const std::string data_file = "aol_with_lru";
  ...
  strawman_hist =
      new StrawmanHistogram<WINDOW_SIZE, 10, strawman_bit_width>(2);
  auto lfu_cache = new LfuCache(CACHE_SIZE, strawman_hist, WINDOW_SIZE);
  ...
  tiny_hist = new TinyLfuHistogram<WINDOW_SIZE, bit_width_tiny>(3);
  lfu_cache = new LfuCache(CACHE_SIZE, tiny_hist, wc_ratio);
  ...
  pure_hist = new PureLfuHistogram();
  lfu_cache = new LfuCache(CACHE_SIZE, pure_hist, 0);
